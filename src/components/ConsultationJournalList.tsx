...
export default function ConsultationJournalList({
  ...
  const [consultationJourData, setConsultationJourData] = useState<ConsultationJournal[]>([]);
  ...
  const fetchConsultationData = async () => {
    ...
    try {
      const response = await fetch('../../public/data/Consulting.json');
      const data = await response.json();
      const filteredData = data.filter((consultation: ConsultationJournal) => consultation.customer_pb_id === pbId);
